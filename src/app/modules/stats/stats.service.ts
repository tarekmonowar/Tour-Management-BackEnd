...
const getTourStats = async () => {
  ...
  const totalHighestBookedTourPromise = Booking.aggregate([
    ...
    //stage-4 lookup stage  Match booking.tour → tour._id	✅ Simple $lookup ----Match grouped._id → tour._id	❌ Can't use simple lookup → Use $expr with let
    {
      $lookup: {
        from: "tours",
        let: { tourId: "$_id" }, // Define variable "tourId" = current _id (which is tour ID)
        pipeline: [
          {
            $match: {
              $expr: { $eq: ["$_id", "$$tourId"] },
